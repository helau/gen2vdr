...
MAX_ROWS=24
MAX_COLS=78
ADM_DIR="/etc/vdr/plugins/admin"
#ADM_DIR="."
ADM_CONF="${ADM_DIR}/admin.conf"
MY_CONF="/tmp/admin.conf.save"
LXDIALOG="lxdialog"
# CONFIG END
...
MAINMENU="${LXDIALOG} --title 'Gen2VDR Einstellungen' --menu '' ${MSIZE} \"\${mainchoice}\" ${mainmenu}"
while : ; do
...
#                                    if [ -z "${value[$subchoice]}" ] ; then
      ...
      1) if [ "$(diff -uN "$ADM_CONF" "$MY_CONF")" != "" ] ; then
            ${LXDIALOG} --title "G2V-Setup" --clear --yesno "Sollen die Einstellungen gesichert werden ?" 5 50
            EXIT_CODE=$?
...
glogger -s "Sichere ${ADM_CONF} als ${ADM_CONF}.bak"
cp -af "${ADM_CONF}" "${ADM_CONF}.bak"
cp "$MY_CONF" "$ADM_CONF"

${ADM_DIR}/setvdrconf.sh

ALL_SCRIPTS=$(diff "${ADM_CONF}" "${ADM_CONF}.bak" | grep "^< /" | grep -v "setvdrconf\.sh" | cut -b 3- | cut -d ":" -f 1 | sort -u)
...
for i in $ALL_SCRIPTS ; do
   if [ -f ${i} ] ; then
      ...
      glogger -s "<${i}> not found"
   fi
done
${LXDIALOG} --title "G2V-Setup" --clear --yesno "Soll VDR neu gestartet werden ?" 5 50
EXIT_CODE=$?
